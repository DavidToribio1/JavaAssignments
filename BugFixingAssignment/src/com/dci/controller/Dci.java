...
public class Dci {
	...
	static boolean findInstructorByName(String input) {
		...
		for (int i = 0; i <= instructors.length - 1; i++) {
			...
			if (instructors[i].getName().equalsIgnoreCase(input)) {
				found = true;
				break;
			}
		}
	...
	static void coursesOfferedByEachInstructor() {
		...
		for(int i =0; i<=instructors.length -1; i++) {
			...
			for(int j=0; j<=instructors[i].coursesOffered.length-1;j++) {
				
				System.out.println(instructors[i].coursesOffered[j]);
			}
		}
	...
	static void instructors1to5Certs() {
		...
		for (int i = 0; i <= instructors.length - 1; i++) {
			...
			if (instructors[i].getCertificationEarned() >= 1 && instructors[i].getCertificationEarned() <= 5) {
	...
	System.out.println("Instructors email");
	String email = input.nextLine();
	
	FullStackDeveloperInstructor instructor1 = new FullStackDeveloperInstructor(name,years, years, years, email, years, args);
